...
{
	if ((position is not integer) || (position != KeyPosition.DOWN && position != KeyPosition.UP))
	{
		raise_exception(location . " requires a position that is from KeyPosition.<variable>")
	}
	return position
}
...
validate_key_arg(location,key)
{
	...
	{
		raise_exception(location . " requires a string key name or a Key() as its argument.")
	}
	return key
}
...
KeySeq(state_array)
{
	...
	Loop, % state_array.MaxIndex()
	{
		...
		{
			raise_exception("KeySeq() requires its array members to be KeyState() objects.")
		}		
	}
